...
本地视频、音频、OCR/ASR 结果和模型默认保存在本机。扩展的 AI 功能是否向用户配置的模型服务发送字幕或问题，取决于用户主动配置的服务；本地 OCR/ASR 引擎不要求上传媒体文件。配置 Supadata 后，YouTube 视频地址会发送到 Supadata 获取原生字幕，API Key 只保存在本机设置中。
