...
var BILI_API = (() => {
  ...
  // 本地缓存和笔记仓储还会服务 YouTube 官方字幕；用带命名空间的 source id
  // 复用既有数据管线，避免把 YouTube 视频伪装成 BV 号。
  function parseSourceId(input) {
    const bvid = parseBvid(input);
    if (bvid) return bvid;
    const text = String(input || "").trim();
    const namespaced = text.match(/^youtube:([A-Za-z0-9_-]{11})$/i);
    if (namespaced) return `youtube:${namespaced[1]}`;
    try {
  ...
  function canonicalVideoUrl(bvid, seconds, page = 1) {
    const sourceId = String(bvid || "").trim();
    const youtube = sourceId.match(/^youtube:([A-Za-z0-9_-]{11})$/i);
    if (youtube) {
      const url = new URL(`https://www.youtube.com/watch?v=${youtube[1]}`);
