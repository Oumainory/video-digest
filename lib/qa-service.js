...
var BILI_QA_SERVICE = (() => {
  ...
  function createQaService({
    ...
    // 标题、创作者和简介是页面直接提供的视频元数据。允许模型用它们回答
    // 「哪个公司/谁发布/叫什么」这类事实题，但只在回答里出现了元数据中的
    // 专有名词时放行，避免把模型随口编的无时间戳回答当成可靠结论。
    function hasMetadataSupport(answer, videoInfo) {
      const source = [videoInfo?.title, videoInfo?.owner, videoInfo?.description]
        .filter(Boolean)
        .join(" ")
        .toLocaleLowerCase();
      const text = String(answer || "").toLocaleLowerCase();
      if (!source || !text) return false;

      const latinTerms = text.match(/[a-z0-9][a-z0-9._-]{2,}/g) || [];
      if (latinTerms.some((term) => source.includes(term))) return true;

      const genericTerms = new Set([
        "视频", "公司", "游戏", "频道", "创作者", "发布", "旗下", "来自", "关于",
        "内容", "一个", "这个", "属于", "作品", "项目",
      ]);
      const cjkRuns = text.match(/[\u3400-\u4dbf\u4e00-\u9fff]{2,}/g) || [];
      for (const run of cjkRuns) {
        for (let length = 2; length <= Math.min(6, run.length); length += 1) {
          for (let start = 0; start + length <= run.length; start += 1) {
            ...
            if (!genericTerms.has(term) && source.includes(term)) return true;
          }
        }
      }
      return false;
    ...
    async function askQuestion({
      ...
      try {
        ...
        const evidence = String(parsed?.evidence || "").toLowerCase();
        const metadataSupported =
          (evidence === "metadata" || evidence === "mixed") &&
          hasMetadataSupport(answer, transcript.videoInfo);
        if (!answer) {
          answer = FALLBACK_ANSWER;
        } else if (!citations.length && !metadataSupported) {
          // 没有字幕引用时，只有模型明确标记为元数据依据且正文出现了
          // 元数据中的专有名词才能通过；其它回答仍视为没有依据。
          answer = FALLBACK_ANSWER;
        }
        ...
        return saveEntry({
          ...
          llmAssistantContent: JSON.stringify({ answer, evidence }),
        });
      } catch (error) {
