...
var BILI_QA_RETRIEVAL = (() => {
  ...
  /**
   * 给「定位字幕」模式使用：始终返回真正命中问题内容的分块，不因为整段
   * 字幕较短就把整段视频全部塞进结果。相邻块只作为上下文补一块。
   */
  function findRelevantChunks(
    chunks,
    question,
    { chapters = [], maxChunks = DEFAULT_MAX_CHUNKS } = {},
  ) {
    ...
    const picked = new Set(scored.slice(0, maxChunks).map((entry) => entry.chunk.index));
