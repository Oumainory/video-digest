...
# 正式发行时把安装包下载页编进扩展；源码包保持空值，避免指向未知站点。
if [ -n "${VIDEO_DIGEST_COMPANION_DOWNLOAD_URL:-}" ]; then
  node scripts/write-companion-release.cjs \
    "$stage/lib/companion-release.js" \
    "$VIDEO_DIGEST_COMPANION_DOWNLOAD_URL" \
    "$version"
fi
