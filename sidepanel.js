...
const state = {
  ...
  companionStatus: { installed: false, running: false, models: [], activeTask: null },
  localResults: [],
  localSelectedSourceId: null,
  localSelectedTrack: "ocr",
};
...
function updatePlatformTabs() {
  for (const button of document.querySelectorAll('.tab[data-tab="overview"], .tab[data-tab="notes"], .tab[data-tab="qa"]')) {
    button.hidden = false;
  }
}
...
function companionRequestResult(response) {
  if (response?.success === false) {
    return {
      success: false,
      error: response.error || "COMPANION_REQUEST_FAILED",
      message: response.message || "桌面软件请求失败。",
    };
  }
  return { success: true, ...(response || {}) };
}

function companionContext(tab) {
  const info = state.data?.videoInfo || {};
  return {
    context: {
      kind: state.platform || (state.bvid ? "bilibili" : "extension"),
      bvid: state.bvid || "",
...
async function loadCompanionStatus({ quiet = true } = {}) {
  try {
    const response = await sendToBackground(
      { action: "getCompanionStatus" },
      { idempotent: true },
    );
    const result = companionRequestResult(response);
    if (!result.success) throw Object.assign(new Error(result.message), { code: result.error });
    state.companionStatus = BILI_COMPANION.normalizeStatus(result.status || result);
  } catch (error) {
    state.companionStatus = {
      ...
      models: [],
      activeTask: null,
      message: error.message || "没有检测到桌面软件。",
    };
    if (!quiet && state.tab === "local") showToast(state.companionStatus.message);
  }
  if (state.tab === "local") renderLocalPanel();
  return state.companionStatus;
}

async function loadLocalTranscripts() {
  try {
    const response = await sendToBackground(
      { action: "listLocalTranscripts" },
      { idempotent: true },
    );
    if (!response?.success) throw new Error(response?.message || "本地结果读取失败。");
    state.localResults = (response.results || [])
      .map((result) => {
        try {
          return BILI_COMPANION.normalizeTranscript(result);
        } catch (error) {
          return null;
        }
      })
      .filter(Boolean);
    if (
      state.localSelectedSourceId &&
      !state.localResults.some((result) => result.sourceId === state.localSelectedSourceId)
    ) {
      state.localSelectedSourceId = null;
    }
  } catch (error) {
    state.localResults = [];
    if (state.tab === "local") showToast(error.message || "本地结果读取失败。");
  }
  if (state.tab === "local") renderLocalPanel();
  return state.localResults;
}
...
function localTrackLabel(kind, track) {
  const name = kind === "ocr" ? "画面字幕 OCR" : "语音 ASR";
  const language = track?.languageLabel || track?.language;
  return language ? `${name} · ${language}` : name;
}

function appendLocalTranscriptViewer(parent, result, kind) {
  const track = result.tracks?.[kind];
  if (!track?.segments?.length) return;
  const viewer = document.createElement("div");
  viewer.className = "local-result-viewer";
  for (const segment of track.segments) {
    const row = document.createElement("div");
    row.className = "segment";
    const time = document.createElement("span");
    time.className = "segment-time";
    time.textContent = BILI_COMPANION.formatTimestamp(segment.start);
...
function renderLocalPanel() {
  const status = BILI_COMPANION.normalizeStatus(state.companionStatus);
  const badge = el("companionStatusBadge");
  badge.textContent = status.installed
    ? status.running
      ? "已连接"
      : "已安装"
    : "未安装";
  badge.classList.toggle("badge-accent", status.installed);
  badge.classList.toggle("badge-muted", !status.installed);

  const cardTitle = el("companionCardTitle");
  const cardText = el("companionCardText");
  const openButton = el("openCompanionBtn");
  const downloadButton = el("downloadCompanionBtn");
  const retryButton = el("companionRetryBtn");
  if (status.installed) {
    cardTitle.textContent = "继续使用桌面识别软件";
    cardText.textContent = state.bvid
      ? `当前${state.platform === "youtube" ? " YouTube" : " B 站"}视频的信息会随本次打开一起带过去；视频文件、播放器和识别过程都在桌面软件中完成。`
      : "在桌面软件中选择本地视频或音频，完成识别后可以从这里查看回传结果。";
    openButton.textContent = "打开软件";
    openButton.hidden = false;
    downloadButton.hidden = true;
    retryButton.hidden = true;
  } else {
    cardTitle.textContent = "需要安装桌面识别软件";
    cardText.textContent = BILI_COMPANION_RELEASE.hasDownloadUrl()
      ? "本地视频和音频不会在扩展里处理。请先下载安装官方桌面软件，安装后回来点击重新检测。"
      : "本地视频和音频不会在扩展里处理。正式发行包地址尚未配置。";
    openButton.hidden = true;
    downloadButton.hidden = false;
    downloadButton.disabled = !BILI_COMPANION_RELEASE.hasDownloadUrl();
    retryButton.hidden = false;
  }

  const task = status.activeTask;
  const taskCard = el("companionTaskCard");
  taskCard.hidden = !task || ["completed", "canceled", "failed"].includes(task.state);
  if (task && !taskCard.hidden) {
    el("companionTaskPercent").textContent = `${task.percent}%`;
    el("companionTaskMessage").textContent = task.message || task.phase || "正在处理…";
    el("companionTaskProgressBar").style.width = `${task.percent}%`;
    el("pauseCompanionTaskBtn").textContent = task.state === "paused" ? "继续" : "暂停";
    el("pauseCompanionTaskBtn").disabled = !["running", "paused"].includes(task.state);
  }
  el("retryCompanionTaskBtn").hidden = task?.state !== "failed";

  const list = el("localResultList");
  list.textContent = "";
  const results = state.localResults || [];
  el("localResultEmpty").hidden = results.length > 0;
  if (results.length && !state.localSelectedSourceId) {
    state.localSelectedSourceId = results[0].sourceId;
  }
  const selectedResult = results.find((result) => result.sourceId === state.localSelectedSourceId);
  if (selectedResult && !selectedResult.tracks?.[state.localSelectedTrack]?.segments?.length) {
    state.localSelectedTrack = Object.keys(selectedResult.tracks || {})[0] || "ocr";
  }
  for (const result of results) {
    const card = document.createElement("article");
    card.className = "local-result-card";
    const head = document.createElement("div");
    head.className = "local-result-card-head";
    const title = document.createElement("p");
    title.className = "local-result-title";
    title.textContent = result.title;
    const date = document.createElement("span");
    date.className = "muted-small";
    date.textContent = formatLocalDate(result.updatedAt);
    head.append(title, date);
    const meta = document.createElement("p");
    meta.className = "local-result-meta";
    meta.textContent = [result.fileName, result.context?.title && `关联：${result.context.title}`]
      .filter(Boolean)
      .join(" · ");
    const tracks = document.createElement("div");
    tracks.className = "local-track-row";
    const available = Object.entries(result.tracks || {})
      .filter(([, track]) => track?.segments?.length);
    for (const [kind, track] of available) {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "local-track-btn";
      button.classList.toggle(
        "active",
        state.localSelectedSourceId === result.sourceId && state.localSelectedTrack === kind,
      );
      button.textContent = localTrackLabel(kind, track);
      const count = document.createElement("span");
      count.className = "local-track-count";
      count.textContent = String(track.segments.length);
      button.appendChild(count);
      button.addEventListener("click", () => {
        state.localSelectedSourceId = result.sourceId;
        state.localSelectedTrack = kind;
        renderLocalPanel();
      });
      tracks.appendChild(button);
    }
    card.append(head, meta, tracks);
    if (state.localSelectedSourceId === result.sourceId) {
      appendLocalTranscriptViewer(card, result, state.localSelectedTrack);
    }
...
async function openCompanion() {
  const button = el("openCompanionBtn");
  const original = button.textContent;
  button.disabled = true;
  button.textContent = "正在打开…";
  try {
    const result = companionRequestResult(
      await sendToBackground({
        action: "openCompanion",
        payload: companionContext(await activeTab()),
      }),
    );
    if (!result.success) {
    ...
    showToast(error.message || "没有检测到桌面识别软件。");
  } finally {
...
async function downloadCompanion() {
  if (!BILI_COMPANION_RELEASE.hasDownloadUrl()) {
    showToast("官方桌面软件安装包地址尚未配置。");
    return;
  }
  await chrome.tabs.create({ url: BILI_COMPANION_RELEASE.DOWNLOAD_URL });
}

async function controlCompanionTask(action) {
  const taskId = state.companionStatus?.activeTask?.id;
  if (!taskId) return;
  const response = await sendToBackground({ action, payload: { taskId } });
  const result = companionRequestResult(response);
  if (!result.success) showToast(result.message);
  else await loadCompanionStatus();
}
...
async function syncWithActiveTab({ force = false, tabId = null } = {}) {
  ...
  const platform = bvid ? "bilibili" : "youtube";
  const page = bvid ? parsePage(tab.url) : 1;
  const unchanged = platform === state.platform
    && sourceId === state.bvid
...
function currentSourceIdentity() {
  return `${state.platform}:${state.bvid || state.youtubeId || ""}:${state.page}`;
}
...
async function loadTranscript({ force = false } = {}) {
  const key = `${currentSourceIdentity()}:${force ? "force" : "normal"}`;
  ...
  const promise = (async () => {
    ...
    const requestedYoutubeId = state.youtubeId;
    const requestedPage = state.page;
    const requestedPlatform = state.platform;
    const requestedIdentity = currentSourceIdentity();
    ...
    try {
      ...
      if (currentSourceIdentity() !== requestedIdentity) return;
      setView("error", backgroundErrorResult(error));
      return;
    }

    if (
      currentSourceIdentity() !== requestedIdentity
      || state.youtubeId !== requestedYoutubeId
    ) return;
...
function exportTranscript() {
  ...
  const sourceUrl = state.data.videoInfo?.url
    || (state.bvid ? `https://www.bilibili.com/video/${state.bvid}` : "");
...
function setupEventListeners() {
  el("refreshBtn").addEventListener("click", () => {
    if (state.bvid || state.youtubeId) loadTranscript({ force: true });
  });
  el("localBtn").addEventListener("click", () => switchTab("local"));
  ...
  el("refreshCompanionBtn").addEventListener("click", () => {
    loadCompanionStatus({ quiet: false });
    loadLocalTranscripts();
  });
  el("companionRetryBtn").addEventListener("click", () => loadCompanionStatus({ quiet: false }));
  el("openCompanionBtn").addEventListener("click", openCompanion);
  el("downloadCompanionBtn").addEventListener("click", downloadCompanion);
  el("pauseCompanionTaskBtn").addEventListener("click", () => {
    const action = state.companionStatus?.activeTask?.state === "paused"
      ? "resumeLocalTask"
      : "pauseLocalTask";
    controlCompanionTask(action);
  });
  el("cancelCompanionTaskBtn").addEventListener("click", () => controlCompanionTask("cancelLocalTask"));
  el("retryCompanionTaskBtn").addEventListener("click", () => controlCompanionTask("retryLocalTask"));
  ...
  chrome.runtime.onMessage.addListener((message) => {
    ...
    if (message?.action === "companionTaskChanged") {
      state.companionStatus = {
        ...state.companionStatus,
        activeTask: BILI_COMPANION.normalizeTask(message.task),
        running: true,
        installed: true,
      };
      if (state.tab === "local") renderLocalPanel();
    }
    if (message?.action === "companionStatusChanged") {
      state.companionStatus = BILI_COMPANION.normalizeStatus(message.status);
      if (state.tab === "local") renderLocalPanel();
    }
    if (message?.action === "localTranscriptReady") {
      let result = null;
      try {
        result = BILI_COMPANION.normalizeTranscript(message.result);
      } catch (error) {
        showToast("收到的本地识别结果无效。");
      }
      if (result) {
        state.localResults = [
          result,
          ...state.localResults.filter((item) => item.sourceId !== result.sourceId),
        ];
        state.localSelectedSourceId = result.sourceId;
        state.localSelectedTrack = Object.hasOwn(result.tracks, state.localSelectedTrack)
          ? state.localSelectedTrack
          : Object.keys(result.tracks)[0] || "ocr";
        showToast("本地识别结果已回传");
        if (state.tab === "local") renderLocalPanel();
      }
    }
    if (message?.action === "localTranscriptError") {
      showToast(message.error || "本地识别结果保存失败。");
    }
