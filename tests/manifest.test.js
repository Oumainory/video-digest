...
test("页面脚本引用的全局模块都有对应的 script 标签", () => {
  // BILI_QA_CITATIONS -> qa-citations.js；设置模块和伴生软件模块是特例。
  const globalToFile = (name) => ({
    BILI_SETTINGS: "settings.js",
    BILI_COMPANION: "companion-protocol.js",
    BILI_COMPANION_BRIDGE: "companion-bridge.js",
    BILI_COMPANION_RELEASE: "companion-release.js",
    BILI_LOCAL_TRANSCRIPT_STORE: "local-transcript-store.js",
  }[name] || `${name.slice("BILI_".length).toLowerCase().replace(/_/g, "-")}.js`);
  ...
  for (const page of ["sidepanel.html", "options.html"]) {
    ...
    const loadedFiles = new Set(
      scripts.map((file) =>
        path.posix.normalize(file.replaceAll("\\", "/")).split("/").pop(),
      ),
    );
...
test("安装时索要的权限只覆盖官方字幕所需平台，且全部走 https", () => {
